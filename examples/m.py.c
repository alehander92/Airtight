...
AList_int a_f_map_intREFint_AList_int_AList_int(int (*a_f)(int), AList_int a_s){
    AList_int a_out = AList_intOf(0);
    for(int i=0;i<a_length_AList_int_int(a_s);i++){
        int a_i = a_index_AList_int_int_int(a_s, i);
        a_out = a_append_AList_int_int_AList_int(a_out, (*a_f)(a_i));
    }
;
    return a_out;
}

int a_nope_int_int(int a_a){
    return (a_a + 4);
}

int main(){
    a_print_AList_int_AString(a_f_map_intREFint_AList_int_AList_int(&a_nope_int_int, AList_intOf(2, 2, 4)));
    return 0;
}
